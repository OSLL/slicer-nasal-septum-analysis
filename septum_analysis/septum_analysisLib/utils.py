...
def getIjkFromRAS(volumeNode, ras):
    volumeRasToIjk = vtk.vtkMatrix4x4()
    volumeNode.GetRASToIJKMatrix(volumeRasToIjk)
    point_Ijk = [0, 0, 0, 1]
    volumeRasToIjk.MultiplyPoint(np.append(ras, 1.0), point_Ijk)
    return np.array([int(round(c)) for c in point_Ijk[:3]])


def getRASFromIjk(volumeNode, ijk):
    volumeIjkToRas = vtk.vtkMatrix4x4()
    volumeNode.GetIJKToRASMatrix(volumeIjkToRas)
    point_Ras = [0, 0, 0, 1]
    volumeIjkToRas.MultiplyPoint(np.append(ijk, 1.0), point_Ras)
    return np.array((point_Ras[0], point_Ras[1], point_Ras[2]))
...
# Taken from https://stackoverflow.com/a/32857432
